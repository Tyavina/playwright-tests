...
export class MainPage {
  ...
  async checkElementsHrefAttribute() {
    for (const { locator, name, attribute } of this.elements) {
      if (attribute) {
        await test.step(`Проверка названия href элемента ${name}`, async () => {
  ...
  async clickSwitchLightModeIcon() {
    await this.page.getByRole('button', { name: 'Switch between dark and light mode (currently system mode)' }).click();
  }
